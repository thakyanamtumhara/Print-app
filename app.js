...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function displayFileFromBase64(name, mimeType, base64Data) {
    ...
    if (mimeType.startsWith('image/')) {
      ...
      // Render HTML content in an iframe, capture as image for preview/print
      console.log('[PRINT-APP] Rendering HTML content: ' + name);
      contentType = 'image';
      pageImages = [];
      pageOrientations = [];
      labelContainer.innerHTML = '<div style="padding:16px;text-align:center;color:#888;font-size:14px">Rendering...</div>';

      try {
        var htmlStr = decodeURIComponent(escape(atob(base64Data)));
        var iframe = document.createElement('iframe');
        iframe.style.cssText = 'position:fixed;left:-9999px;top:-9999px;width:794px;border:none;';
        document.body.appendChild(iframe);
        iframe.contentDocument.open();
        iframe.contentDocument.write(htmlStr);
        iframe.contentDocument.close();

        // Remove any onclick="print()" from body to prevent auto-print
        if (iframe.contentDocument.body) {
          iframe.contentDocument.body.removeAttribute('onclick');
          iframe.contentDocument.body.removeAttribute('onload');
        }
        ...
        setTimeout(function() {
          try {
            ...
            var contentHeight = Math.max(body.scrollHeight, body.offsetHeight, docEl.scrollHeight, docEl.offsetHeight);
            iframe.style.height = contentHeight + 'px';
            console.log('[PRINT-APP] HTML iframe rendered, contentHeight=' + contentHeight);

            // Use SVG foreignObject to capture
            var serialized = new XMLSerializer().serializeToString(iframe.contentDocument.documentElement);
            var svgStr = '<svg xmlns="http://www.w3.org/2000/svg" width="794" height="' + contentHeight + '">' +
            ...
            img.onload = function() {
              ...
              labelContainer.innerHTML =
                '<div style="padding:12px;text-align:center">' +
                '<img src="' + dataUrl + '" style="max-width:100%;border-radius:4px;object-fit:contain">' +
                '</div>';
              resizeCanvas();
              console.log('[PRINT-APP] HTML rendered to image successfully');
            };

            img.onerror = function() {
              console.error('[PRINT-APP] SVG foreignObject render failed, showing in iframe');
              URL.revokeObjectURL(svgUrl);
              // Fallback: show HTML directly in an embedded iframe
              document.body.removeChild(iframe);
              var displayIframe = document.createElement('iframe');
              displayIframe.style.cssText = 'width:100%;height:100%;border:none;border-radius:6px;background:#fff;';
              displayIframe.srcdoc = htmlStr;
              labelContainer.innerHTML = '';
              labelContainer.appendChild(displayIframe);
              contentType = 'label';
              resizeCanvas();
            };
            ...
            console.error('[PRINT-APP] HTML render error:', renderErr);
            document.body.removeChild(iframe);
        ...
        console.error('[PRINT-APP] HTML decode error:', e);
        labelContainer.innerHTML =
          '<div style="padding:32px 16px;text-align:center;color:#c00">' +
          '<div style="font-size:15px;font-weight:600">Could not render HTML</div>' +
          '<div style="font-size:13px;margin-top:4px;color:#888">' + (e.message || e) + '</div></div>';
      }
    } else {
      contentType = 'label';
      pageImages = [];
  ...
  // ── Load file from URL params (works with TWA, new tabs, mobile) ──
  (function checkUrlParams() {
    var params = new URLSearchParams(window.location.search);
    var hash = window.location.hash;

    // Method 1: ?proxy=PROXY_URL&name=FILENAME — fetch PDF via proxy
    if (params.get('proxy')) {
      var proxyUrl = params.get('proxy');
      var fileName = params.get('name') || 'document.pdf';
      console.log('[PRINT-APP] Loading from proxy URL param:', fileName);
    ...
    if (params.get('url')) {
      ...
      var fileName = params.get('name') || 'document.pdf';
      console.log('[PRINT-APP] Loading from URL param:', fileName);
      fetchAndDisplayUrl(fileName, fileUrl);
      window.history.replaceState({}, '', window.location.pathname);
      return;
    }

    // Method 3: #printdata=JSON — base64 data in hash
    if (hash && hash.indexOf('#printdata=') === 0) {
      try {
        var jsonStr = decodeURIComponent(hash.substring('#printdata='.length));
        var data = JSON.parse(jsonStr);
        console.log('[PRINT-APP] Loading from hash data:', data.fileName, data.mimeType);
        displayFileFromBase64(data.fileName, data.mimeType, data.base64Data);
      } catch (e) {
        console.error('[PRINT-APP] Error parsing hash data:', e);
      }
      window.history.replaceState({}, '', window.location.pathname);
      return;
    }
  ...
  // ── Fetch from CORS proxy (JSON response with base64) ──
  function fetchProxyAndDisplay(fileName, proxyUrl) {
    console.log('[PRINT-APP] fetchProxyAndDisplay:', fileName, proxyUrl);
  ...
  // ── Receive file from external apps via postMessage (desktop fallback) ──
  window.addEventListener('message', function(event) {
    try {
      var data = event.data;
      if (data && data.type === 'PRINT_APP_LOAD_FILE') {
        console.log('[PRINT-APP] Received file via postMessage: ' + data.fileName + ' (' + data.mimeType + ')');
        displayFileFromBase64(data.fileName, data.mimeType, data.base64Data);
      } else if (data && data.type === 'PRINT_APP_LOAD_URL') {
        console.log('[PRINT-APP] Received URL via postMessage: ' + data.fileName);
        fetchAndDisplayUrl(data.fileName, data.fileUrl);
      }
    } catch (e) {
      console.error('[PRINT-APP] Error handling postMessage:', e);
    }
  });

  // ── Fetch file from URL and display ──
  function fetchAndDisplayUrl(fileName, fileUrl) {
    ...
    fetch(fileUrl)
      ...
      .catch(function(e) {
        ...
        console.log('[PRINT-APP] Trying pdf.js direct URL load...');
        // Method 2: Try loading via pdf.js directly (it can handle CORS in some cases)
        if (typeof pdfjsLib !== 'undefined') {
          pdfjsLib.getDocument(fileUrl).promise.then(function(pdf) {
            console.log('[PRINT-APP] pdf.js loaded URL directly, pages=' + pdf.numPages);
            clearWhiteout();
            eraserEverUsed = false;
            zoom = 1;
            applyZoom();
            renderPdfFromPdfObj(pdf);
          }).catch(function(pdfErr) {
            console.error('[PRINT-APP] pdf.js URL load also failed:', pdfErr.message);
            console.log('[PRINT-APP] Falling back to iframe embed');
            // Method 3: Fallback to iframe embed with explicit height
            contentType = 'label';
            pageImages = [];
            pageOrientations = [];
            var containerHeight = labelContainer.parentElement ? labelContainer.parentElement.offsetHeight : 500;
            var iframeHeight = Math.max(containerHeight - 20, 400);
            var displayIframe = document.createElement('iframe');
            displayIframe.style.cssText = 'width:100%;height:' + iframeHeight + 'px;border:none;border-radius:6px;background:#fff;';
            displayIframe.src = fileUrl;
            labelContainer.innerHTML = '';
            labelContainer.appendChild(displayIframe);
            resizeCanvas();
          });
          ...
          console.log('[PRINT-APP] pdf.js not available, iframe fallback');
          contentType = 'label';
          pageImages = [];
          pageOrientations = [];
          var containerHeight = labelContainer.parentElement ? labelContainer.parentElement.offsetHeight : 500;
  ...
  // Helper: render a pdf.js PDFDocumentProxy into preview (used by fetchAndDisplayUrl)
  function renderPdfFromPdfObj(pdf) {
    labelContainer.innerHTML = '';
    pageImages = [];
    pageOrientations = [];
    contentType = 'pdf';
    ...
    function renderPage(num) {
      ...
      pdf.getPage(num).then(function(page) {
        var vp = page.getViewport({ scale: 1 });
        var scale = Math.min(4960 / vp.width, 10);
        console.log('[RENDER] Page ' + num + ': vp=' + vp.width.toFixed(0) + 'x' + vp.height.toFixed(0) + ' scale=' + scale.toFixed(2));
        var scaled = page.getViewport({ scale: scale });
  ...
  // Signal to opener (Dashboard) that Print App is ready to receive files
  if (window.opener) {
    try {
      console.log('[PRINT-APP] Signaling READY to opener');
      window.opener.postMessage({ type: 'PRINT_APP_READY' }, '*');
    } catch (e) {
      console.log('[PRINT-APP] Could not signal opener:', e.message);
    }
  }
});
